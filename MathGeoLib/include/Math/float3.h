...
public:
	...
	float3 planeProj_xy() const { return float3(x, y, 0); } ///< [similarOverload: xxx] [hideIndex]
	float3 planeProj_xz() const { return float3(x, 0, z); } ///< [similarOverload: xxx] [hideIndex]
	float3 planeProj_yz() const { return float3(0, y, z); } ///< [similarOverload: xxx] [hideIndex]

	float3 planeProj(float3 vec1, float3 vec2) 
	{
		float3 normal = vec1.Cross(vec2);
		float3 projection = (normal* float3(x, y, z).Dot(normal)) / normal.LengthSq();
