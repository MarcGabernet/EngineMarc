...
void Frustum::SetType(FrustumType type) {
	this->type = type;
}

void Frustum::SetFOV(float horizontalFov)
{
	this->horizontalFov = horizontalFov;
}

void Frustum::SetAspectRatio(float aspect, int width, int height)
{
	this->verticalFov = 2.0f * atanf(tanf(horizontalFov * 0.5f) * aspect * (float)height / (float)width);
}

void Frustum::SetPlaneDistances(float nearPlane, float farPlane)
{
	this->nearPlaneDistance = nearPlane;
	this->farPlaneDistance = farPlane;
}

void Frustum::SetPosition(float3 pos)
{
	this->pos = pos;
}

void Frustum::SetFront(float3 front)
{
	this->front = front.Normalized();
}

void Frustum::SetUp(float3 up)
{
	this->up = up.Normalized();
}

Frustum::Frustum(FrustumType type, float3 position, float3 up, float3 front, float nearPlaneDistance, float farPlaneDistance, float horizontalFov)
{
	this->type = type;
	this->pos = position;
	this->up = up.Normalized();
	...
	//This will be changed with the aspect, this is a placeholder value
	this->verticalFov = 1;
}
