...
public:
	...
	Frustum() { }
	Frustum(FrustumType type, float3 position, float3 up, float3 front, float nearPlaneDistance, float farPlaneDistance, float horizontalFov);
	...
	void SetFOV(float horizontalFOV); //set horizontal FOV 
	void SetAspectRatio(float aspect, int width, int height); //change vertical FOV to match horizontal
